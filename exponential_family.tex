...
\begin{alphabate} 
\item Set $h(x) \equiv 1$ if we change $\mu$ to $\wt{\mu}$, chosen so that its Radon-Nikodym derivative with respect to $\mu$ is $h$.\footnote{That is, we can always absorb $h$ into $\mu$ and write \eqref{eqn:exponential_family_natural} as  $p_\eta(x) = \exp \{ \eta^\top \sufficientStatsFunction(x) - \logNormalizerFunction(\eta)\}$, in which case \eqref{eqn:partition_function} becomes $Z(\eta) := \ds\int  \exp \{ \eta^\top \sufficientStatsFunction(x)  \} \; \wt{\mu} (dx) $ where $\wt{\mu}(A) := \ds\int_A h d\mu$ for any measurable set $A$. }
...
\item Change $h$ to $\wt{h}(x) = p_0(x)$, assuming that $0 \in H$.  Then, changing $\logNormalizerFunction(\eta)$ to $\wt{\logNormalizerFunction}(\eta) = \logNormalizerFunction(\eta) - \logNormalizerFunction(0)$ results in an equivalent family. 
\end{alphabate}
...
\label{eqn:exponential_family}
 p_\theta(x) = h(x) \exp \{ \eta(\theta)^\top \sufficientStatsFunction(x) - \logNormalizerFunction(\eta(\theta))\} 
 \end{align}
...
\[p(x \cond \pi) = \prod_{k=1}^K \pi_k^{x_k} = \exp \{ \sum_{k=1}^K x_k \log \pi_k\} \]
 with natural parameter, $\eta(\pi) = \log \pi$, the sufficient statistics $\sufficientStatsFunction(x) = x$, carrier density $h(x)=1$ and log normalizer $0$.
\end{example}
...
\end{align*}
with natural parameter $\eta(\alpha) = [\alpha_1 -1, ..., \alpha_K -1]^\top$, sufficient statistics $\sufficientStatsFunction(\pi) = \log \pi = [\log \pi_1, ..., \log \pi_K]^\top$, carrier density $h(\pi)=1$, and log normalizer $\logNormalizerFunction(\alpha) =  \sum_k \log \Gamma (\alpha_k ) - \log \Gamma (\sum_k \alpha_k)$. 
...
\end{align*} 
where $K := \big( \Phi_{\mu, \sigma^2} (b)  - \Phi_{\mu, \sigma^2} (a) \big)^{-1}$,  then we see that see that  $\TruncatedNormal(\mu,  \sigma^2,  (a,b))$ belongs to the exponential family \eqref{eqn:exponential_family} where,  in this case,  we have natural parameter $\eta = (\frac{1}{\sigma^2},  \frac{\mu}{\sigma^2})\top$,  sufficient statistics function $\sufficientStatsFunction(x) = (-\half x^2,  x)^\top$,  carrier density $h(x) =  \frac{1}{\sqrt{2 \pi}}  \indicate{a \leq x \leq b}$,  and log normalizer $a(\theta) = \log K + \frac{\mu^2}{\sigma^2} - \log \sigma$.
 
\end{example}

\begin{remark}
The truncated normal distribution differs from the normal distribution only in its carrier density $h(x)$ and therefore log normalizer $\logNormalizerFunction(\theta)$.  The natural parameter $\eta$ and sufficient statistics function $\sufficientStatsFunction(x)$ are identical.   Thus,  knowing $\eta$ and $\sufficientStatsFunction(x)$ is not sufficient to determine the form of the probability distribution.
...
\end{align*}
with natural parameter $\eta(\alpha) = [-\alpha-1,  -\beta]\top$, sufficient statistics $\sufficientStatsFunction(x) = [\log x ,  \frac{1}{x}]^\top$, carrier density $h(x)=1$, and log normalizer $a(\alpha,  \beta) =  \log \frac{\beta^\alpha}{\Gamma(\alpha)} $. 
...
\end{align*} 
with natural parameter $\eta(\+\mu, \+\Sigma) = \big(-\half \vectorize (\+\Sigma^{-1}) , \,  \+\Sigma^{-1} \+\mu \big) $, sufficient statistics $\sufficientStatsFunction(\+x) = \big(\vectorize (\+x \+x^\top), \+x \big)$, carrier density $h(x) = (2\pi)^{-d/2}$ and log normalizing $\logNormalizerFunction(\+\mu, \+\Sigma) =  -\half \+\mu \+\Sigma^{-1} \+\mu + \half \log |\+\Sigma^{-1} | $. 
\end{example}
...
As we see from the last line,  in the exponential family representation,  we have natural parameter $\eta = [\frac{-(\nu + p + 1)}{2},  \; -\half \vectorize (\+\Psi)]^\top$, sufficient statistics $\sufficientStatsFunction(\+X) = [\log | \+X | , \vectorize (\+X^{-1})]^\top$, carrier density $h(\+X)=1$, and log normalizer $ \log  C(\+\Psi,  \nu)$.   
...
Since $X$ is in the exponential family, its density can be written in the form
 \[ p(x \cond \eta) =  \exp \{ \eta^\top \sufficientStatsFunction(x) - \logNormalizerFunction(\eta) + \log h(x)\} \] 
where 
\[\logNormalizerFunction(\eta) = \log \int_{\mathcal{X}} \exp \{ <\sufficientStatsFunction(x), \eta> + \log h(x) \} \; d\nu_{\mathcal{X}} \] 

Thus
\begin{align*}
\nabla \logNormalizerFunction (\eta) & \stackrel{1}{=}  \df{ \int_{\mathcal{X}} \sufficientStatsFunction(x)  \exp \{ <\sufficientStatsFunction(x), \eta> + \log h(x) \} \; d\nu_{\mathcal{X}} }{\int_{\mathcal{X}} \exp \{ <\sufficientStatsFunction(x), \eta> + \log h(x) \} \; d\nu_{\mathcal{X}} } \\
& \stackrel{2}{=} \int_{\mathcal{X}} \sufficientStatsFunction(x)  \exp \{ <\sufficientStatsFunction(x), \eta> - \logNormalizerFunction(\eta) + \log h(x) \} \; d\nu_{\mathcal{X}}  \\
...
\begin{subequations}
\begin{align}
\H[\eta]&= \logNormalizerFunction(\eta) - \bigip{\eta}{\nabla \logNormalizerFunction(\eta)} - \E_{\eta} [\log h(X)]	\label{eqn:entropy_of_an_exponential_family}\\
\H[\eta, \wt{\eta}] &=  \logNormalizerFunction(\wt{\eta}) - \bigip{\wt{\eta}}{\nabla \logNormalizerFunction(\eta)} - \E_{\eta}  [\log h(X)] \label{eqn:cross_entropy_between_two_exponential_family_members}
\end{align}
...
The natural parameter is $\eta = - \frac{1}{\sigma}$, the log normalizer is $\logNormalizerFunction(\eta) = \log (-\frac{2}{\eta})$ (and so $\nabla \logNormalizerFunction(\eta) = -\frac{1}{\eta}$), and the carrier density is $h(x)=1$ (which allows us to use \eqref{eqn:entropy_of_an_exponential_family_with_base_measure_unity} instead of \eqref{eqn:entropy_of_an_exponential_family}).    
...
\label{eqn:exponential_family_iid}
 p(\+x \cond \theta) = \ds\prod_{i=1}^n h(x_i) \exp \big\{ \eta(\theta)^\top \sum_{i=1}^n \sufficientStatsFunction(x_i) - n \,\logNormalizerFunction(\eta(\theta))\big\} 
 \end{align}
...
\[ \mu(\theta_{ML}) = \df{1}{n} \ds\sum_{i=1}^n \sufficientStatsFunction(x_i) \]
where $\mu := \E[\sufficientStatsFunction(X)]$ refers to the mean parametrization of the likelihood. \redfootnote{TODO: This switching of parameterization should be handled much more explicitly.}
...
\label{eqn:exponential_family_complete_data_likelihood}
 p(\+x, \+z \cond \theta) = \ds\prod_{i=1}^n h(x_i, z_i) \exp \big\{ \eta(\theta)^\top \sum_{i=1}^n \sufficientStatsFunction(x_i, z_i) - n \,\logNormalizerFunction(\eta(\theta))\big\} 
 \end{align}
...
\[ \mu(\theta^{(t+1)}) = \df{1}{n} \ds\sum_{i=1}^n   \E_{p(\+z \cond \+x , \+\theta^{(t)})} \sufficientStatsFunction(x_i, z_i) \]
where $\mu := \E[\sufficientStatsFunction(x_1,z_1)]$ refers to the mean parametrization of the likelihood.
...
\begin{proof}
Let $f_{(r,p)}$ be the density of a Negative Binomial random variable, $g_\lambda$ be the density of a Poisson random variable and $h_{(\alpha, \beta)}$ be the density of a Gamma random variable.   Then we must show that 
...
\[ p(x \cond \eta) = h(x) \exp \{ \eta^\top T(x) - A(\eta) \} \]
...
\[  \bigg\{\eta : \ds\int h(x) \exp \{ \eta^\top T(x) - A(\eta) \} < \infty \bigg \}\]

Given a random sample, $\+x=(x_1, x_2, .., x_N)$, we obtain:
\[ p(\+x \cond \eta) = \bigg( \ds\prod_{i=1}^N h(x_i)  \bigg) \exp \bigg\{ \eta^\top  \ds\sum_{i=1}^N T(x_i) - N A(\eta) \bigg\} \]
