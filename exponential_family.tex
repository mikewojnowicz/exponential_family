...
\subsubsection{Gradient of the Q-function}
 ...
 Here, we give the gradient of the Q-function in Prop.~\ref{prop:gradient_of_the_Q_function}. Using that, we can provide an interesting representation of the EM step \Eqref{eqn:EM_algorithm} in Prop.~\ref{prop:EM_update}. 
...
\red{TODO: It might be possible to give a much faster proof using the idea of EM as exact CAVI inference.}
...
There exists a symmetric positive define \enquote{transformation matrix} $\transformationMatrixGradientToEMStep(\+\theta^^t)$ such that the EM step $\+\theta^^{t-1} - \+\theta^^t$ can be expressed as
%
\begin{align}
\+\theta^^{t+1} - \+\theta^^t = \transformationMatrixGradientToEMStep(\+\theta^^t) \frac{\partial}{\partial \+\theta} \ell(\+\theta) \Bigr|_{\+\theta=\+\theta^^t}
...
%
\begin{align}
\bigg[\overline{T}^{-1} \overline{T}_z(\+\theta^^t) - \+\theta^^t \bigg] \stackrel{?}{=} \transformationMatrixGradientToEMStep(\+\theta^^t)  \bigg[ \overline{T}_z(\+\theta^^t)  - 	\overline{T}(\+\theta^^t) \bigg]
...
\begin{proof}
Since the matrix $\transformationMatrixGradientToEMStep$ in Prop.~\ref{prop:relating_the_EM_step_to_the_ordinary_gradient} is symmetric positive definite, the EM step $\+\theta^^{t+1} - \+\theta^^t$ is an ascent direction by \citet[pp.31, Eqn. (3.2)]{nocedal2006numerical}.  Namely, using shorthand notation $\nabla \ell_t \defeq \frac{\partial }{\partial \+\theta} \ell(\+\theta) \Bigr|_{\+\theta = \+\theta^^t}$, we compute the directional derivative as
%
...
\subsubsection{Convergence behavior of the EM algorithm}

Here, we discuss the convergence behavior of the EM algorithm, following \citet{salakhutdinov2002relationship} and \citet{salakhutdinov2003optimization}.

The EM algorithm implicitly defines a mapping $\EMmap : \paramSpace \to \paramSpace$ where $\EMmap(\+\param^^t) = \+\param^^{t+1}$.

\red{TODO: Use this material to address some of the questions at the end of the section on relating the EM step and the ordinary gradient.}
