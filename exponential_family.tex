...
for some functions $\carrierDensity, \eta, \sufficientStatsFunction, \logNormalizerFunction$. 
 ...
 In the special case where $(\+x, \+z)=((x_1,z_1), ..., (x_n, z_n))$ are $n$ i.i.d samples from some exponential family, we can write
%
\begin{align}
...
\label{eqn:exponential_family_complete_data_likelihood_based_on_iid_draws}
 \end{align}
%
for some functions $\carrierDensityForSingleSample, \eta, \sufficientStatsFunctionForSingleSample, \logNormalizerFunctionForSingleSample$ describing the exponential family for a single observation. 
...
The expectation maximization objective is to update parameters $\+\theta$ by iteratively solving
%
\begin{equation}
...
\red{TODO: Justify why we are optimizing THIS particular function; one justification comes from a special case of CAVI, but it would be interesting to see the justification that does not use that.}   \blue{DISCUSS: Does Prop~\ref{prop:Q_function_and_log_marginal_likelihood_have_the_same_gradients} give a/the justification?} 
...
We will provide a solution to the EM objective of \Eqref{eqn:em_objective_on_parameters} in Prop.~\ref{prop:EM_update}. First, we give the gradient of the Q-function in Prop.~\ref{prop:gradient_of_the_Q_function}. Before that, we provide some justification for optimizing the Q-function in Prop~\ref{prop:Q_function_and_log_marginal_likelihood_have_the_same_gradients}.
...
\end{align*}
%
\label{prop:Q_function_and_log_marginal_likelihood_have_the_same_gradients}
\end{proposition}
...
\begin{remark}\remarktitle{The derivative as the difference in expected sufficient statistics with clamped and unclamped data.}
By Prop.~\ref{prop:gradient_of_the_Q_function},
%
...
\begin{notation}
The interpretation of the RHS of \Eqref{eqn:gradient_of_Q_function_for_EM} was given in Notation~\ref{notation:Q_function}.     Using the alternative notation of \citet{miller2011why} that we mentioned in Notation~\ref{notation:Q_function}, we could also write \Eqref{eqn:gradient_of_Q_function_for_EM} as 
 %
 \begin{align}
...
\begin{proof}
We find a local optimum of the Q-function by finding its critical point, that is by solving $\frac{\partial Q(\+\theta \cond \+\theta^^t)}{\partial \+\theta} =  0$. \red{TODO: How do we know that we don't need to look at boundary conditions?  How do we know that the optimum is a maximum and not a minimum?}   We apply the representation of $\frac{\partial Q(\+\theta \cond \+\theta^^t)}{\partial \+\theta}$ in \Eqref{eqn:gradient_of_Q_function_for_EM_in_jeff_millers_notation}, set $\+\theta=\+\theta^^{t+1}$, and add the same term to both sides to obtain the result.
\end{proof}


\subsubsection{...for i.i.d data}
