...
\textbf{What justifies this particular algorithm?}  One way to justify EM is as a special case of Coordinate Ascent Variational Inference (CAVI) \cite{wojnowiczXXXXvariational}. In particular, suppose we have a frequentist latent variable model (with observed data $\+x$, random latent variables $\+z$, and fixed but unknown parameters $\+\theta$) for which we can compute the exact posterior.  In this setting, an unrestricted variational family is formed by $q=q_\+z q_\+\theta$  where $q_\+\theta = \delta_\+\theta$.  Now at the $t$-th iteration of coordinate ascent, we do:
...
\item \textit{Update to $q_\+\theta$.}	 To update $q_\+\theta$, we note that the variational lower bound has the form
%
\begin{align}
...
We will provide a solution to the EM objective of \Eqref{eqn:M_step_for_EM} in Prop.~\ref{prop:EM_update}. First, we give the gradient of the Q-function in Prop.~\ref{prop:gradient_of_the_Q_function}. %Before that, we provide some justification for optimizing the Q-function in Prop~\ref{prop:Q_function_and_log_marginal_likelihood_have_the_same_gradients_when_evaluated_at_the_current_param_value}.


\begin{proposition}\propositiontitle{The gradient of the Q-function \cite{salakhutdinov2002relationship}.}
 The gradient of the Q-function of \Eqref{eqn:E_step_for_EM}
is given by 
 %
 \begin{align}
...
_{p(\+x,\+z \cond \+\theta)} [\sufficientStatsFunction(\+x, \+z)] =  \int \sufficientStatsFunction(\+x, \+z) p(\+x,\+z \cond \+\theta) \mu(d\+x, d\+z).$ In (2), we used the derivative of the logarithm along with chain rule for derivatives. In (3) we substituted the result of the first line. In (4), we used the definition in \Eqref{eqn:E_step_for_EM}
...
The solution to the EM objective in \Eqref{eqn:M_step_for_EM} for obtaining the next parameter value $\+\theta^^{t+1}$ given the previous parameter value $\+\theta^^{t}$ is given by finding the $\+\theta^^{t+1}$ that solves
%
\begin{align}
