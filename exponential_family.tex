...
\[ \nabla \logNormalizerFunction(\naturalParam) = \E[\sufficientStatsFunction(X)] \]
%
For minimal families (Def.~\ref{def:minimal_exponential_families}), it turns out that this relationship is invertible \cite[Sec.~8.4]{jordan2010exponential}. \red{TODO: Prove this.}  This implies that a distribution in the exponential family can be parameterized not only by $\naturalParam$ -- the canonical parametrization -- but also by $\meanParameter$ -- the \textit{mean parametrization}. \red{TODO: Prove that we can always reparametrize a non-minimal EF into a minimal EF.}  
...
_{p(\+x,\+z \cond \param)} [\sufficientStatsFunction(\+x, \+z)] =  \int \sufficientStatsFunction(\+x, \+z) p(\+x,\+z \cond \param) \mu(d\+x, d\+z).$ In (2), we used the derivative of the logarithm along with chain rule for derivatives. In (3) we substituted the result of the first line. In (4), we used the definition in \Eqref{eqn:E_step_for_EM}
...
The solution to the EM objective in \Eqref{eqn:M_step_for_EM} for obtaining the next parameter value $\param^^{t+1}$ given the previous parameter value $\param^^{t}$ is given by finding the $\param^^{t+1}$ that solves
%
\begin{align}
\E_{\param^^{t+1}}[\sufficientStatsFunction(\+X,\+Z)] = \E_{\param^^t}[\sufficientStatsFunction(\+X,\+Z) \cond \+X=\+x] 
...
\begin{proof}
We find a local optimum of the Q-function by finding its critical point, that is by solving $\frac{\partial Q(\param \cond \param^^t)}{\partial \param} =  0$. \red{TODO: How do we know that we don't need to look at boundary conditions?  How do we know that the optimum is a maximum and not a minimum?}   By applying the representation of $\frac{\partial Q(\param \cond \param^^t)}{\partial \param}$ in \Eqref{eqn:gradient_of_Q_function_for_EM_in_jeff_millers_notation}, we see that this is equivalent to finding $\param \in \param$ which solves the equation
%
\begin{align*}
\E_{\param}[\sufficientStatsFunction(\+X,\+Z)] = \E_{\param^^t}[\sufficientStatsFunction(\+X,\+Z) \cond \+X=\+x] 	
\end{align*}
%
Setting the solution $\param^*$ as the next parameter value $\param^^{t+1} = \param^*$ gives the result. \red{TODO: How do we know the solution exists?}
\end{proof}
...
There exists a symmetric positive define \enquote{transformation matrix} $\transformationMatrixGradientToEMStep(\param^^t)$ such that the EM step $\param^^{t-1} - \param^^t$ can be expressed as
%
\begin{align}
...
\end{align}
%
Using these same definitions, the EM update of \Eqref{eqn:EM_update_to_parameters} is given by $\param^^{t+1}$ which solves
%
...
%
\begin{align}
 p(\+x, \+z \cond \param) = \bigg( \ds\prod_{i=1}^n \carrierDensityForSingleSample(x_i, z_i) \bigg) \exp \big\{ \naturalParam(\param)^\top \sum_{i=1}^n \sufficientStatsFunctionForSingleSample(x_i, z_i) - n \,\logNormalizerFunctionForSingleSample(\naturalParam(\param))\big\} 
\label{eqn:exponential_family_complete_data_likelihood_based_on_iid_draws}
 \end{align}
%
for some functions $\carrierDensityForSingleSample, \naturalParam, \sufficientStatsFunctionForSingleSample, \logNormalizerFunctionForSingleSample$ describing the exponential family for a single observation (the so-called \textit{unit complete data likelihood}). 

Following the logic of Section \ref{sec:ml_with_ef}, we expect that in the i.i.d case, we should select $\param^{(t+1)}$ such that
%
\begin{align}
\meanParameter(\param^{(t+1)}) = \df{1}{n} \ds\sum_{i=1}^n   \E_{p(\+z \cond \+x , \param^{(t)})} \sufficientStatsFunction(x_i, z_i) 
