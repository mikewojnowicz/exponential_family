...
Some models have latent variables associated with each observation.   Let us  assume that $(\+x, \+z)=((x_1,z_1), ..., (x_n, z_n))$ are n independent samples from an  exponential family, where $\+x$ is observed data and $\+z$ is unobserved data.
%
\begin{align}
 p(\+x, \+z \cond \theta) = \ds\prod_{i=1}^n \carrierDensity(x_i, z_i) \exp \big\{ \eta(\theta)^\top \sum_{i=1}^n \sufficientStatsFunction(x_i, z_i) - n \,\logNormalizerFunction(\eta(\theta))\big\} 
\label{eqn:exponential_family_complete_data_likelihood}
 \end{align}
%
In this case, we say that the \textit{complete data likelihood} is an exponential family.

\subsection{Expectation Maximization}

When a model contains latent variables as in \Eqref{eqn:exponential_family_complete_data_likelihood}, it is not possible to use the maximum likelihood strategy of Sec.~\ref{sec:ml_with_ef} directly. Indeed, the maximum likelihood solution of \Eqref{eqn:maximum_likelihood_solution_for_exponential_families} cannot be applied because it is a function of unobserved variables $\+z$.  However, when complete data likelihood is an exponential family as in \Eqref{eqn:exponential_family_complete_data_likelihood}, we can perform frequentist inference via Expectation Maximization (EM).  
...
We see how this plays out in exponential families by following the logic of Section \ref{sec:ml_with_ef}.   Here we want to find $\+\theta$ to optimize
% 
\[ Q(\+\theta \cond \+\theta^^t) =  \E_{p(\+z \cond \+x , \+\theta^^t)} \bigg[ \ln p(\+x, \+z \cond \+\theta) \bigg] \]
%
...
where $\meanParameter := \E[\sufficientStatsFunction(x_1,z_1)]$ refers to the mean parametrization (Sec.~\ref{sec:mean_parametrization}) of the likelihood.
