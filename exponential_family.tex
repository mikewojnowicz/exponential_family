...
The natural parameter is $\eta = - \frac{1}{\sigma}$, the log normalizer is $a(\eta) = \log (-\frac{2}{\eta})$ (and so $\nabla a(\eta) = -\frac{1}{\eta}$), and the base measure is $h(x)=1$.    Using this, we can easily compute 
\begin{align*}
\H[\eta]&= a(\eta) - \bigip{\eta, \nabla a(\eta)} - \cancelto{0}{\E_{\eta} [\log h(X)]} \\
&= \log \bigg(-\frac{2}{\eta} \bigg)  - \bigip{\eta, - \frac{1}{\eta}}  =\log \bigg(-\frac{2}{\eta} \bigg) + 1 \\ 
...
Let 
\begin{align*}
p(x \cond \eta) &=  \exp \{ \eta^T t(x) - a(\eta) + \log \wt{h}(x)\}  \\
...
So there are now two potentially problematic terms, although in the case where $h(x) \equiv 1$, only one potentially problematic term remains.
\end{remark}
