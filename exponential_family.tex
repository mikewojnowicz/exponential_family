...
\begin{remark}\remarktitle{Exponential families with general (non-natural) parametrizations.}
...
\begin{remark}\remarktitle{The Hessian of the log normalizer equals the covariance of the sufficient statistics.}
...
\red{TODO: I think I can give a much faster proof using the idea of EM as exact CAVI inference.}
...
\begin{proposition}
Under a minimal exponential family model with mean parametrization, the EM step is identical to gradient ascent  of the log marginal likelihood with step size of 1.  That is, $\transformationMatrixGradientToEMStep(\param^^t) = \+I$ in Prop.~\ref{prop:relating_the_EM_step_to_the_ordinary_gradient}.
\label{prop:when_EM_is_exactly_gradient_ascent}
\end{proposition}

\begin{proof}[Proof sketch.]
%
\begin{align*}
\frac{\partial }{\partial \param} \ell(\param) \Bigr|_{\param = \param^^t} &= \E_{\param^^t}[\sufficientStatsFunction(\+X,\+Z) \cond \+X=\+x] - 	\E_{\param^^t}[\sufficientStatsFunction(\+X,\+Z)] && \scripttext{Cor.~\ref{cor:ordinary_gradient_of_log_marginal_likelihood_of_an_exponential_family_latent_variable_model}} \\
&=\E_{\param^^{t+1}}[\sufficientStatsFunction(\+X,\+Z)] - 	\E_{\param^^t}[\sufficientStatsFunction(\+X,\+Z)] && \scripttext{Prop.~\ref{prop:interesting_representation_of_EM_update_to_params} (EM step)} \\
...
\red{TODO: Use this material to address some of the questions at the beginning of this section.}
