...
\newcommand{\carrierDensityForSingleSample}{\wt{h}}
...
\begin{alphabate} 
\item Set $\carrierDensity(x) \equiv 1$ if we change $\mu$ to $\wt{\mu}$, chosen so that its Radon-Nikodym derivative with respect to $\mu$ is $\carrierDensity$.\footnote{That is, we can always absorb $\carrierDensity$ into $\mu$ and write \eqref{eqn:exponential_family_natural} as  $p_\eta(x) = \exp \{ \eta^\top \sufficientStatsFunction(x) - \logNormalizerFunction(\eta)\}$, in which case \eqref{eqn:partition_function} becomes $Z(\eta) := \ds\int  \exp \{ \eta^\top \sufficientStatsFunction(x)  \} \; \wt{\mu} (dx) $ where $\wt{\mu}(A) := \ds\int_A \carrierDensity d\mu$ for any measurable set $A$. }
...
\begin{definition}
An exponential family is said to be \textbf{minimal} if the components of the sufficient statistics $\sufficientStatsFunction(x)$ are linearly independent ($\mu$-a.e.).\redfootnote{CHECK: This statement, when given by David Blei, made no mention of almost everywhere.  The next statement, however, which came from \cite{johnson2016composing}, does.  I attempted to align them by adding ``almost everywhere" to the linear independence claim.  Hopefully this is valid.}  That is, there must be no $\eta(\theta) \in \R^n \textbackslash \set{0}$ such that $\eta(\theta)^\top \sufficientStatsFunction(x) = 0$ ($\mu$-a.e.).\redfootnote{Is it strictly speaking necessary to assume that the parameters are real-valued? If so, why?}
...
\red{TODO: I believe a non-minimal EF can always be reparametrized into a minimal EF. This makes sense based on the definition, based on the categorical distribution, and based on an implication from \citet[Sec.~8.4]{jordan2010exponential}.  Justify this.}
...
\begin{proposition}\propositiontitle{The gradient of the log normalizer equals the expected sufficient statistics.}
...
\begin{align*}
p(x \cond \eta) &=  \exp \{ \eta^\top \sufficientStatsFunction(x) - \logNormalizerFunction(\eta) + \log \carrierDensity(x)\}  \\
\wt{p}(x \cond \wt{\eta}) &=  \exp \{ \wt{\eta}^\top \wt{\sufficientStatsFunction}(x) - \wt{\logNormalizerFunction}(\wt{\eta}) + \log \wt{h}(x)\}  \\	
\end{align*}
...
&= -\ds\int p(x \cond \eta) \bigg[ \bigip{\wt{\sufficientStatsFunction}(x)}{\wt{\eta}} - \wt{a} (\wt{\eta}) + \log \wt{h}(x)  \bigg] dx \\
&= \wt{a}(\wt{\eta})- \bigip{ \E_p[\wt{\sufficientStatsFunction}(x)]}{\wt{\eta}} - \E_p[\log \wt{h}(x)]
\end{align*}
...
\subsection{Mean parametrization} \label{sec:mean_parametrization}
...
\[ \nabla \logNormalizerFunction(\eta) = \E[\sufficientStatsFunction(X)] \]
%
For minimal families (Def.~\ref{def:minimal_exponential_families}), it turns out that this relationship is invertible \cite[Sec.~8.4]{jordan2010exponential}. \red{TODO: Prove this.}  This implies that a distribution in the exponential family can be parameterized not only by $\eta$ -- the canonical parametrization -- but also by $\meanParameter$ -- the \textit{mean parametrization}. \red{TODO: Prove that we can always reparametrize a non-minimal EF into a minimal EF.}  
...
\end{align}
%
where $\meanParameter:= \E[\sufficientStatsFunction(X)]$ refers to the mean parametrization (Sec.~\ref{sec:mean_parametrization}) of the likelihood.  \redfootnote{TODO: This switching of parameterization should be handled more constructively.}
...
When a model contains latent variables as in \Eqref{eqn:exponential_family_complete_data_likelihood_based_on_iid_draws}, it is not possible to use the maximum likelihood strategy of Sec.~\ref{sec:ml_with_ef} directly. Indeed, the maximum likelihood solution of \Eqref{eqn:maximum_likelihood_solution_for_exponential_families} cannot be applied because it is a function of unobserved variables $\+z$.  However, when complete data likelihood is an exponential family as in \Eqref{eqn:exponential_family_complete_data_likelihood_based_on_iid_draws}, we can perform frequentist inference via Expectation Maximization (EM).   We see how this plays out in exponential families by following the logic of Section \ref{sec:ml_with_ef}; see \citet[Sec.~3]{salakhutdinov2002relationship} and \citet{miller2011why} for useful references. 

\subsubsection{The EM step}

The expectation maximization procedure is to update parameters $\+\theta$ by iteratively solving
%
\begin{equation}
 \+\theta^{(t+1)} =  \text{argmax}_{\+\theta} \; \E_{p(\+z \cond \+x , \+\theta^{(t)})} \bigg[ \ln p(\+x, \+z \cond \+\theta) \bigg] 
\label{eqn:em_objective_on_parameters}
 \end{equation}
 %
In particular, at the $(t+1)$-st iterate we seek to find $\+\theta$ which optimizes the so-called \textit{Q-function}
% 
\begin{align}
 Q(\+\theta \cond \+\theta^^t) &\defeq  \E_{p(\+z \cond \+x , \+\theta^^t)} \bigg[ \ln p(\+x, \+z \cond \+\theta) \bigg] 
 \label{eqn:Q_function_for_EM}
\end{align}
%

\red{TODO: Justify why we are optimizing THIS particular function; one justification comes from a special case of CAVI, but it would be interesting to see the justification that does not use that.}   \blue{DISCUSS: Does Prop~\ref{prop:Q_function_and_log_marginal_likelihood_have_the_same_gradients_when_evaluated_at_the_current_param_value} give a/the justification?} 

\begin{notation}\remarktitle{Notation for expected values.}
In the Q-function of \Eqref{eqn:Q_function_for_EM} above, our notation means
%
...
\[ \E_{\+\theta^^t} \bigg[ \ln p_{\+\theta}(\+X, \+Z) \cond \+X=\+x \bigg].\]
\label{notation:Q_function}
\end{notation}


We will provide a solution to the EM objective of \Eqref{eqn:em_objective_on_parameters} in Prop.~\ref{prop:EM_update}. First, we give the gradient of the Q-function in Prop.~\ref{prop:gradient_of_the_Q_function}. %Before that, we provide some justification for optimizing the Q-function in Prop~\ref{prop:Q_function_and_log_marginal_likelihood_have_the_same_gradients_when_evaluated_at_the_current_param_value}.


\begin{proposition}\propositiontitle{The gradient of the Q-function \cite{salakhutdinov2002relationship}.}
 The gradient of the Q-function of \Eqref{eqn:Q_function_for_EM} is given by 
 %
 \begin{align}
...
_{p(\+x,\+z \cond \+\theta)} [\sufficientStatsFunction(\+x, \+z)] =  \int \sufficientStatsFunction(\+x, \+z) p(\+x,\+z \cond \+\theta) \mu(d\+x, d\+z).$ In (2), we used the derivative of the logarithm along with chain rule for derivatives. In (3) we substituted the result of the first line. In (4), we used the definition in \Eqref{eqn:Q_function_for_EM} of the Q-function, along with an (assumed) interchange of the integral and the derivative. In (5), we used linearity of the integral; in the second summand, we have a constant with respect to the density $p(\+z \cond \+x, \+\theta^^t)$, which integrates to 1.  In (6), we simply applied notation.  
\end{proof}
...
\begin{proposition}\propositiontitle{EM update to parameters.}
The solution to the EM objective in \Eqref{eqn:em_objective_on_parameters} for obtaining the next parameter value $\+\theta^^{t+1}$ given the previous parameter value $\+\theta^^{t}$ is given by finding the $\+\theta^^{t+1}$ that solves
%
\begin{align}
\E_{\+\theta^^{t+1}}[\sufficientStatsFunction(\+X,\+Z)] = \E_{\+\theta^^t}[\sufficientStatsFunction(\+X,\+Z) \cond \+X=\+x] 
\label{eqn:EM_update_to_parameters}	
\end{align}
%
...
\label{prop:EM_update}
\end{proposition}

\begin{proof}
We find a local optimum of the Q-function by finding its critical point, that is by solving $\frac{\partial Q(\+\theta \cond \+\theta^^t)}{\partial \+\theta} =  0$. \red{TODO: How do we know that we don't need to look at boundary conditions?  How do we know that the optimum is a maximum and not a minimum?}   By applying the representation of $\frac{\partial Q(\+\theta \cond \+\theta^^t)}{\partial \+\theta}$ in \Eqref{eqn:gradient_of_Q_function_for_EM_in_jeff_millers_notation}, we see that this is equivalent to finding $\+\theta \in \+\Theta$ which solves the equation
%
\begin{align*}
\E_{\+\theta}[\sufficientStatsFunction(\+X,\+Z)] = \E_{\+\theta^^t}[\sufficientStatsFunction(\+X,\+Z) \cond \+X=\+x] 	
\end{align*}
%
Setting the solution $\+\theta^*$ as the next parameter value $\+\theta^^{t+1} = \+\theta^*$ gives the result. \red{TODO: How do we know the solution exists?}
\end{proof}

\subsubsection{Relating the EM step and the ordinary gradient}


\begin{proposition}\propositiontitle{The Q-function and log marginal likelihood have equal gradients \cite{salakhutdinov2002relationship}.}
Consider a latent variable model whose complete data likelihood  $p(\+x, \+z \cond \+\theta)$ is in the exponential family.  Define its log marginal likelihood as
% 
\begin{align}
\ell(\+\theta) \defeq \log p(\+x \cond \+\theta) = \log \int p(\+x,\+z \cond \+\theta) \mu(d\+z).	
...
%
\begin{align}
\frac{\partial }{\partial \+\theta} \ell(\+\theta) \Bigr|_{\+\theta = \+\theta^^t} = \frac{\partial }{\partial \+\theta} Q(\+\theta \cond \+\theta^^t) \Bigr|_{\+\theta = \+\theta^^t}.
\end{align}
%
...
%
\begin{align*}
\frac{\partial }{\partial \+\theta} \ell(\+\theta) \Bigr|_{\+\theta = \+\theta^^t} = \frac{\partial }{\partial \+\theta} Q(\+\theta \cond \+\theta^^t) \Bigr|_{\+\theta = \+\theta^^t} \bigg( = \frac{1}{p(\+x \cond \+\theta^^t)} \;  \int \frac{\partial}{\partial \+\theta}  p(\+x, \+z \cond \+\theta) \Bigr|_{\+\theta=\+\theta^^t} \, \mu(d\+z) \bigg)
\end{align*}
as claimed.

\end{proof}

\begin{corollary}\propositiontitle{The ordinary gradient of the log marginal likelihood of an exponential family latent variable model.}

We have 
%
\begin{align}
\frac{\partial }{\partial \+\theta} \ell(\+\theta) \Bigr|_{\+\theta = \+\theta^^t} = \E_{\+\theta^^t}[\sufficientStatsFunction(\+X,\+Z) \cond \+X=\+x] - 	\E_{\+\theta^^t}[\sufficientStatsFunction(\+X,\+Z)]
...
\begin{proof}
This follows immediately from the gradient of the Q-function (see \Eqref{eqn:the_gradient_of_the_Q_function_evaluated_at_the_current_parameter_and_using_jeffs_notation}) along with the fact that the Q-function and the log marginal likelihood have equal gradients when evaluated at the current parameter value (Prop.~\ref{prop:Q_function_and_log_marginal_likelihood_have_the_same_gradients_when_evaluated_at_the_current_param_value}).	
\end{proof}


\begin{proposition}\propositiontitle{Relating the EM step to the ordinary gradient \cite{salakhutdinov2002relationship}.}
There exists a symmetric positive define matrix $\+P(\+\theta^^t)$ such that the EM step $\+\theta^^{t-1} - \+\theta^^t$ can be expressed as
%
\begin{align}
\+\theta^^{t+1} - \+\theta^^t = \+P(\+\theta^^t) \frac{\partial}{\partial \+\theta} \ell(\+\theta) \Bigr|_{\+\theta=\+\theta^^t}
...
%
\begin{align}
\overline{T}(\+\theta) & \defeq \int p(\+z, \+x \cond \+\theta) \sufficientStatsFunction(\+x, \+z) \mu_{\+x,\+z}(d\+x, d\+z) \\
\overline{T}_z(\+\theta) & \defeq \int p(\+z \cond \+x, \+\theta) \sufficientStatsFunction(\+x, \+z) \mu_\+z(d\+z) 
\end{align}
%
...
%
\begin{align}
\frac{\partial }{\partial \+\theta} \ell(\+\theta) \Bigr|_{\+\theta = \+\theta^^t} = \overline{T}_z(\+\theta^^t)  - 	\overline{T}(\+\theta^^t)
...
\end{align}
%
Using these same definitions, the EM update of \Eqref{eqn:EM_update_to_parameters} is given by $\+\theta^^{t+1}$ which solves
%
\[  \overline{T}(\+\theta^^{t+1}) = \overline{T}_z(\+\theta^^t)  \]
%
Since $ \overline{T}$ is an  invertible function (see Sec.~\ref{sec:mean_parametrization}), the EM update is given by
%
\begin{align}
\+\theta^^{t+1} = \overline{T}^{-1} \overline{T}_z(\+\theta^^t)
...
%
\begin{align}
\bigg[\overline{T}^{-1} \overline{T}_z(\+\theta^^t) - \+\theta^^t \bigg] \stackrel{?}{=} \+P(\+\theta^^t)  \bigg[ \overline{T}_z(\+\theta^^t)  - 	\overline{T}(\+\theta^^t) \bigg]
...
\end{align}
%
we can further simplify \Eqref{eqn:relating_the_EM_step_to_the_ordinary_gradient__compact_notation}.  In particular, we must show that there exists symmetric positive definite matrix $\+P$ such that
%
\begin{align}
\+v(\+\theta^^t) \stackrel{?}{=} \+P(\+\theta^^t)  \+u(\+\theta^^t)
...
%
\begin{align}
\+P(\+\theta^^t) \defeq  \frac{\+v(\+\theta^^t) \+v(\+\theta^^t)^\top}{\+v(\+\theta^^t)^\top \+u(\+\theta^^t)}.
...
%
\begin{align}
\+P \+u =  \frac{\+v \cancel{\+v^\top}}{\cancel{\+v^\top \+u}} \cancel{\+u} = \+v,
\end{align}
%
as desired.  

Now all that remains is to show that $\+P$ is symmetric positive definite.  It is clear from \Eqref{eqn:solution_to_transformation_matrix_relating_the_EM_step_to_the_ordinary_gradient__VERY_compact_notation} that $\+P$ is symmetric.  We also see from \Eqref{eqn:solution_to_transformation_matrix_relating_the_EM_step_to_the_ordinary_gradient__VERY_compact_notation} that $\+P$ is positive definite if $\+v^\top \+u >0$, and that is shown in \citet[Eqn.~26]{salakhutdinov2002relationship}.
\end{proof}

\begin{corollary}
The EM step gives an ascent direction for the log marginal likelihood.
\end{corollary}

\begin{proof}
The matrix $\+P$ in Prop.~\ref{prop:relating_the_EM_step_to_the_ordinary_gradient} is symmetric positive definite, the EM step $\+\theta^^{t+1} - \+\theta^^t$ is an ascent direction by \citet[pp.31, Eqn. (3.2)]{nocedal2006numerical}.  Namely, using shorthand notation $\nabla \ell_t \defeq \frac{\partial }{\partial \+\theta} \ell(\+\theta) \Bigr|_{\+\theta = \+\theta^^t}$, we compute the directional derivative as
%
\[ \nabla \ell_t^\top (\+\theta^^{t+1} - \+\theta^^t) \stackrel{(Prop.~\ref{prop:relating_the_EM_step_to_the_ordinary_gradient})}{=}  \nabla \ell_t^\top  \+P  \nabla \ell_t \stackrel{p.d.}{>} 0,\]
and hence the EM step $\+\theta^^{t+1} - \+\theta^^t$ is an ascent direction.
%
\end{proof}
...
\item Is there some way to improve EM using ordinary gradient but without bringing in the 2nd order info?  I.e., is there some \textit{other} $\+P$ that would be better? If so, why?
\item  \citet[abstract]{qin2000direct} mentions that (at least in the context of HMMs), EM is especially slow (relative to direct optimization) in the low STN regime.  Why would this be?
...
Also, how does the $\+P$ matrix magically keep update in param bounds?
}

\subsubsection{The EM algorithm for i.i.d data}

In the special case where $(\+x, \+z)=((x_1,z_1), ..., (x_n, z_n))$ are $n$ i.i.d samples from some exponential family, we can write the complete data likelihood of \Eqref{eqn:exponential_family_complete_data_likelihood} as 
%
\begin{align}
 p(\+x, \+z \cond \+\theta) = \bigg( \ds\prod_{i=1}^n \carrierDensityForSingleSample(x_i, z_i) \bigg) \exp \big\{ \eta(\+\theta)^\top \sum_{i=1}^n \sufficientStatsFunctionForSingleSample(x_i, z_i) - n \,\logNormalizerFunctionForSingleSample(\eta(\+\theta))\big\} 
\label{eqn:exponential_family_complete_data_likelihood_based_on_iid_draws}
 \end{align}
%
for some functions $\carrierDensityForSingleSample, \eta, \sufficientStatsFunctionForSingleSample, \logNormalizerFunctionForSingleSample$ describing the exponential family for a single observation (the so-called \textit{unit complete data likelihood}). 

Following the logic of Section \ref{sec:ml_with_ef}, we expect that in the i.i.d case, we should select $\+\theta^{(t+1)}$ such that
%
\begin{align}
\meanParameter(\+\theta^{(t+1)}) = \df{1}{n} \ds\sum_{i=1}^n   \E_{p(\+z \cond \+x , \+\theta^{(t)})} \sufficientStatsFunction(x_i, z_i) 
\label{eqn:EM_iteration_as_maximum_likelihood_with_expected_sufficient_statistics}	
\end{align}
%
where $\meanParameter := \E[\sufficientStatsFunction(x_i,z_i)]$ refers to the mean parametrization (Sec.~\ref{sec:mean_parametrization}) of the \textit{unit complete data likelihood}.  This is true.  \red{TODO: Justify.} \red{TODO: I believe that I read \Eqref{eqn:EM_iteration_as_maximum_likelihood_with_expected_sufficient_statistics} in some paper -- perhaps notes by Jordan.  Find the source so that I can cite it and perhaps provide additional context.} 
...
Note that the update in \Eqref{eqn:EM_iteration_as_maximum_likelihood_with_expected_sufficient_statistics} also corroborates with the more general update (not assuming i.i.d models) given in \Eqref{eqn:EM_update_to_parameters}. \red{TODO: Prove directly that the latter implies the former in the i.i.d case.}
...
\red{TODO: Check this section, especially with respect to the fact that I am dealing with three parametrizations here - $\mu, \theta, \nu$; that is, mean, arbitrary, and natural, respectively.  Really the core problem is that it's not sufficiently clear in how head how and when reparametrizations affect things.}
 ...
 \red{TODO (Notation): Fix notation here and throughout. For instance: (1) As usual, the boldfacing is giving me headaches.  For instance, if we use boldface $\+\theta$ to represent arbitrary params, then we should presumably  use boldface for the natural and mean parameterizations. (2) Be more consistently careful about when we're referring to random variables, and when to realizations.}
