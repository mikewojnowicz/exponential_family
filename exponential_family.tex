...
For a latent variable model of \Eqref{eqn:exponential_family_complete_data_likelihood}  whose complete data likelihood  $p(\+x, \+z \cond \param)$ is in the exponential family, the gradient of the Q-function of \Eqref{eqn:E_step_for_EM}
is given by 
 %
 \begin{align}
...
\begin{remark}\remarktitle{Interpreting the EM step for exponential family models.} Following \citet[pp.4]{dempster1977maximum}, we can interpret the representation in Prop~\ref{prop:interesting_representation_of_EM_update_to_params} in terms of the maximum likelihood updates for completely observed exponential family models (Sec.~\ref{sec:ml_with_ef}). Recall from \Eqref{eqn:condition_for_critical_point_of_EF_likelihood} that in the completely observed setting, maximum likelihood asks us to find the parameter $\param$ that satisfies $\E_{\param}[\sufficientStatsFunction(\+\obsCaps)] = \sufficientStatsFunction(\obs)$.  However, in the latent variable case, we do not completely observe the sufficient statistics $\sufficientStatsFunction(\+X,\+Z)$, and so we instead use the \textit{expected} sufficient statistics $\E_{\param^^t}[\sufficientStatsFunction(\+X,\+Z) \cond \+X=\+x]$ from the most recent parameter estimate. Thus,  \Eqref{eqn:interesting_represenation_of_EM_step_for_exponential_family_models} combines the E-step on the RHS with the M-step on the LHS.
\end{remark}
...
Consider the latent variable model of \Eqref{eqn:exponential_family_complete_data_likelihood}  whose complete data likelihood  $p(\+x, \+z \cond \param)$ is in the exponential family.  Define its log marginal likelihood as
% 
\begin{align}
