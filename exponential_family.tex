...
\end{align}
%
where $\meanParameter:= \E[\sufficientStatsFunction(X)]$ refers to the mean parametrization (Sec.~\ref{sec:mean_parametrization}) of the likelihood.  \redfootnote{TODO: This switching of parameterization should be handled more explicitly.}
...
\label{eqn:em_objective_on_parameters}
 \end{equation}
 %
 Note in particular that we seek to find $\+\theta$ which optimizes the so-called \textit{Q-function}
% 
\begin{align}
...
\end{align}
%
In the above, our notation means
$\E_{p(\+z \cond \+x , \+\theta^^t)} \bigg[ \ln p(\+x, \+z \cond \+\theta) \bigg] \defeq \int \ln p(\+x, \+z \cond \+\theta) \, p(\+z \cond \+x , \+\theta^^t) \, \mu(d\+z)$.
%
\red{TODO: Justify why we are optimizing THIS particular function; one justification comes from a special case of CAVI, but it would be interesting to see the justification that does not use that.}  

\begin{remark}
The $Q$ function in \Eqref{eqn:Q_function_for_EM} is perhaps more clearly denoted by \citet{miller2011why} as 
\[ \E_{\+\theta^^t} \bigg[ \ln p_{\+\theta}(\+X, \+Z) \cond \+X=\+x \bigg].\]
\label{rk:jeff_millers_notation_for_Q_function}
\end{remark}

We will provide a solution to the parameter updates of \Eqref{eqn:em_objective_on_parameters} in Prop.~\ref{prop:EM_update}. First, we give the gradient of the Q-function in Prop.~\ref{prop:gradient_of_the_Q_fucntion}.

\begin{proposition}\propositiontitle{\cite{salakhutdinov2002relationship}.}
 ...
 \end{align}
 %
 which can be interpreted as the difference in the expected sufficient statistic vector when the observed data is clamped and unclamped.
%
\label{prop:gradient_of_the_Q_fucntion}
 \end{proposition}

\begin{remark}
Using the notation of \citet{miller2011why} that we mentioned in Remark~\ref{rk:jeff_millers_notation_for_Q_function}, we could also write \Eqref{eqn:gradient_of_Q_function_for_EM} as 
 %
 \begin{align}
...
\red{TODO: Check this section, especially with respect to the fact that I am dealing with three parametrizations here - $\mu, \theta, \nu$; that is, mean, arbitrary, and natural, respectively.  Really the core problem is that it's not sufficiently clear in how head how and when reparametrizations affect things.}
 ...
 \red{TODO (Notation): Fix notation here and throughout. For instance: (1) As usual, the boldfacing is giving me headaches.  For instance, if we use boldface $\+\theta$ to represent arbitrary params, then we should presumably  use boldface for the natural and mean parameterizations. (2) Be more careful about when we're referring to random variables, and when to realizations.}
