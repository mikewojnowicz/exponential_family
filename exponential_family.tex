...
Since the log-likelihood of a (regular) exponential family is convex \citet[pp.~4]{dempster1977maximum} (\red{TODO: Add justification in my convexity section that the full log likelihood is convex (currently I just have that the log normalizer is convex; probably I just need to apply that convexity is closed under certain arithmetic operations); be sure to pay close attention to where the regularity condition is applied}), solving \Eqref{eqn:condition_for_critical_point_of_EF_likelihood} will find the maximum likelihood value of the density $p_\naturalParam$ unless the maximizing value $\naturalParam$ lies on the boundary of the parameter space $\naturalParamSpace$.
...
\subsubsection{Gradient of the Q-function}
 ...
 Here, we give the gradient of the Q-function in Prop.~\ref{prop:gradient_of_the_Q_function}. Using that, we can provide an interesting representation of the EM step \Eqref{eqn:EM_algorithm} in Prop.~\ref{prop:interesting_representation_of_EM_update_to_params}. 
...
\begin{proposition}\propositiontitle{An interesting representation of the EM update to parameters.}
...
\label{eqn:EM_update_to_parameters__interesting_representation}	
\end{align}
%
...
\begin{remark}\remarktitle{Intuition on convergence of EM algorithm to a local maximum.} Following \citet[pp.~5]{dempster1977maximum}, suppose that the EM algorithm produces a convergent sequence of parameters $\param^^t \to \param^*$ for an exponential family latent variable model (see \Eqref{eqn:exponential_family_complete_data_likelihood}).  Then by Prop.~\ref{prop:interesting_representation_of_EM_update_to_params} we have
	%
\begin{align}
...
\end{align}
%
Using these same definitions, the EM update of \Eqref{eqn:EM_update_to_parameters__interesting_representation} is given by $\param^^{t+1}$ which solves
%
...
The EM algorithm implicitly defines a mapping $\EMmap : \paramSpace \to \paramSpace$ where $\EMmap(\param^^t) = \param^^{t+1}$.   If $\param_t \to \param^*$, and $\EMmap$ is continuous, then $\param^* = \EMmap(\param^*)$, and near $\param^*$, by a Taylor series expansion
%
\begin{align*}
...
\red{TODO: Finish writing up the analysis of \citet{salakhutdinov2003optimization}, which builds off of the Demspter, Laird, and Rubin insight that the convergence rate matrix can be expressed as a ratio of the entropy information to energy information.  They use that to determine when to use EM and when to use a Newton method. Note for the CPD paper that an optimization based approach (what we're selling for the CPD paper) is \textit{critical} for both determining when EM is slow and determining how to fix it.  Without the optimization based approach, all we have is the Q (energy) function, and the H (entropy) term never appears. } 
...
Note that the update in \Eqref{eqn:EM_iteration_as_maximum_likelihood_with_expected_sufficient_statistics} also corroborates with the more general update (not assuming i.i.d models) given in \Eqref{eqn:EM_update_to_parameters__interesting_representation}. \red{TODO: Prove directly that the latter implies the former in the i.i.d case.}
...
\input{inputs/conjugacy_and_semi_conjugacy}
...
\input{inputs/general_conjugacy_formalism}
...
\input{inputs/entropy_facts_about_mvn}
