...
\[p(x \cond \pi) = \prod_{k=1}^K \pi_k^{x_k} = \exp \{ \sum_{k=1}^K x_k \log \pi_k\} \]
 with natural parameter, $\eta(\pi) = \log \pi$, the sufficient statistics $t(x) = x$, base measure $h(x)=1$ and log normalizer $0$.\redfootnote{TODO: Write this up as its own example.}
\end{example}
...
\item The uniform distribution, whose density cannot be written in the form \eqref{eqn:exponential_family}. %(Loosely speaking, its base measure is a function of the parameter.)
\end{itemize}
...
Since $X$ is in the exponential family, its density can be written in the form\footnote{Note that here we let $k(x) = \log h(x)$.}  
...
\subsubsection{Entropy, cross-entropy, and KL divergence between members}
...
\begin{align*}
\texttt{KL}(\wt{\eta} || \eta) = \bigip{\nabla a(\wt{\eta}), \wt{\eta} - \eta} + a(\eta) - a(\wt{\eta}) 
...
\begin{align*}
\texttt{KL}(\wt{\eta} || \eta) &= \ds\int p(x \cond \wt{\eta}) \log \bigg( \df{p(x \cond \wt{\eta})}{p(x \cond \eta)}\bigg) \;dx \\ 
& = \ds\int p(x \cond \wt{\eta}) \; \bigg[ \bigip{t(x), \wt{\eta} - \eta}  + a(\eta) - a(\wt{\eta}) \bigg] \; dx \\
&= \bigip{\E_{\wt{\eta}}[t(X)],  \wt{\eta} - \eta } + a(\eta) - a(\wt{\eta}) \\
...
By reasoning in a similar way as the proof of Proposition \ref{prop:kl_divergence_between_members_of_same_exponential_family}, expressions for the entropy $\H[\eta] = \E_{\eta}[- \log p(\eta)]$ and cross-entropy $\H[\wt{\eta}, \eta] = \E_{\wt{\eta}}[- \log p(\eta)]$ can also be provided:

\begin{subequations}
\begin{align}
\H[\wt{\eta}]&= a(\wt{\eta}) - \bigip{\wt{\eta}, \nabla a(\wt{\eta})} - \E_{\wt{\eta}} [\log h(X)]	\\
\H[\wt{\eta}, \eta] &=  -a(\eta) + \bigip{\eta, \nabla a(\wt{\eta})} + \E_{\wt{\eta}}  [\log h(X)]
\end{align}
\label{eqn:entropy_and_cross_entropy_between_two_members_of_same_exponential_family}
\end{subequations}



Note that unlike with KL divergence \eqref{eqn:KL_divergence_between_two_members_of_the_sample_exponential_family}, the expressions for entropy and cross entropy \eqref{eqn:entropy_and_cross_entropy_between_two_members_of_same_exponential_family} may not have a closed form solution.  However, note that these expressions will always automatically have closed form solution when the base measure satisfies $h(x) \equiv 1$, as is the case, for example, with the Gaussian, Dirichlet, and inverse gamma distributions.
...
\subsubsection{i.i.d samples from an exponential family}
